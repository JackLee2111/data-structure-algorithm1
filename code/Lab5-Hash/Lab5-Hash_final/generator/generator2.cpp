...
#define OUTPUT_POJ "./data/temp.txt"
...
void creat_data_for_poj()
{
    ...
    sort(line1 + 1, line1 + data_num1 + 1, cmp);
    unique(line1 + 1, line1 + data_num1 + 1);
    /*
...
void creat_data_for_hdu()
{
    ...
    srand((unsigned)time(NULL));
    for (int i = 1; i <= 2 * data_num2; i++)
    {
        line2[i] = rand() % MOD;
    }

    sort(line2 + 1, line2 + 2 * data_num2 + 1, cmp);
    unique(line2 + 1, line2 + 2 * data_num2 + 1);

    /*
    for (int i = 1; i <= data_num2; i++)
    {
        cout << line2[i] << " ";
    }
    cout << endl;*/
    ...
    while (!infile2.eof()) // 若未到文件结束一直循环
    {
        ...
        if (i_line2 <= data_num2 && line2[i_line2] == i2)
        {
            ...
            i_line2++;
            // cout << name << " i:" << i2 << " " << number << endl;
        }
        else if (i_line2 > data_num2)
        {
            break;
        }
        /*
